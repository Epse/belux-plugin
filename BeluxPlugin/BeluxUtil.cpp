...
string BeluxUtil::https_fetch_file(string url)
{
	...
	do
	{
		if (InternetReadFile(url_handle, buffer, buffer_size, &bytes_read))
		{
			...
			output = output + string(buffer, bytes_read);
		}
		else
		{
                                                ...
                                                double lon)
{
		...
		weatherdoc != NULL && !weatherdoc.HasParseError())
	{
		...
		double kelvin = stod(
			weatherdoc["data"].GetObject()[closest.c_str()].GetObject()[to_string(FL).c_str()].GetObject()["T(K)"].
			GetString());
		double windheading = stod(
			weatherdoc["data"].GetObject()[closest.c_str()].GetObject()[to_string(FL).c_str()].GetObject()["windhdg"].
			GetString());
		double windspeed = stod(
			weatherdoc["data"].GetObject()[closest.c_str()].GetObject()[to_string(FL).c_str()].GetObject()["windspeed"].
			GetString());
